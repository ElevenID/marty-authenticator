  ...
  group('Migration', () {
    ...
    test('loadTokens ignores invalid entries during migration', () async {
      ...
      final legacyData = {
        ...
        '${legacyPrefix}_invalid_json':
            'this is not a json string', // Invalid JSON
        '${legacyPrefix}_missing_type': jsonEncode({
          'some_key': 'some_value',
        }), // Valid JSON, but not a token
      };
      ...
      when(
        mockStorage.write(key: validNewKey, value: validTokenValue),
      ).thenAnswer((_) async {});
      when(
        mockLegacyStorage.delete(key: validLegacyKey),
      ).thenAnswer((_) async {});
      ...
      verifyNever(
        mockStorage.write(
          key: '${legacyPrefix}_invalid_json',
          value: anyNamed('value'),
        ),
      );
      verifyNever(
        mockLegacyStorage.delete(key: '${legacyPrefix}_invalid_json'),
      );
      verifyNever(
        mockStorage.write(
          key: '${legacyPrefix}_missing_type',
          value: anyNamed('value'),
        ),
      );
      verifyNever(
        mockLegacyStorage.delete(key: '${legacyPrefix}_missing_type'),
      );
