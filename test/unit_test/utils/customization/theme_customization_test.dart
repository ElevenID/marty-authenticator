...
void _testThemeCustomization() {
  group('Theme Customization Test', () {
    ...
    test('constructor', () {
      ...
      expect(
        customization.actionButtonsForegroundColor,
        equals(const Color(0xFF00000D)),
      );
      expect(
        customization.tileDefaultOtpColor,
        equals(const Color(0xFF00000E)),
      );
      expect(
        customization.tileWarningOtpColor,
        equals(const Color(0xFF00001E)),
      );
      expect(
        customization.tileCriticalOtpColor,
        equals(const Color(0xFF00002E)),
      );
      expect(
        customization.tileDefaultCountdownColor,
        equals(const Color(0xFF00003E)),
      );
      expect(
        customization.tileWarningCountdownColor,
        equals(const Color(0xFF00004E)),
      );
      expect(
        customization.tileCriticalCountdownColor,
        equals(const Color(0xFF00005E)),
      );
      expect(customization.tileSubtitleColor, equals(const Color(0xFF00000F)));
      expect(
        customization.navigationBarIconColor,
        equals(const Color(0xFF000010)),
      );
      expect(
        customization.qrButtonBackgroundColor,
        equals(const Color(0xFF000011)),
      );
    ...
    test('copyWith', () {
      ...
      expect(
        newCustomization.navigationBarColor,
        equals(const Color(0xFFFFFFF3)),
      );
      expect(
        newCustomization.actionButtonsForegroundColor,
        equals(const Color(0xFFFFFFF2)),
      );
      expect(
        newCustomization.tileDefaultOtpColor,
        equals(const Color(0xFFFFFFF1)),
      );
      expect(
        newCustomization.tileWarningOtpColor,
        equals(const Color(0xFFFFFFF0)),
      );
      expect(
        newCustomization.tileCriticalOtpColor,
        equals(const Color(0xFFFFFFEF)),
      );
      expect(
        newCustomization.tileDefaultCountdownColor,
        equals(const Color(0xFFFFFFEE)),
      );
      expect(
        newCustomization.tileWarningCountdownColor,
        equals(const Color(0xFFFFFFED)),
      );
      expect(
        newCustomization.tileCriticalCountdownColor,
        equals(const Color(0xFFFFFFEC)),
      );
      expect(
        newCustomization.tileSubtitleColor,
        equals(const Color(0xFFFFFFF0)),
      );
      expect(
        newCustomization.navigationBarIconColor,
        equals(const Color(0xFFFFFFEF)),
      );
      expect(
        newCustomization.qrButtonBackgroundColor,
        equals(const Color(0xFFFFFFEE)),
      );
      expect(
        newCustomization.qrButtonIconColor,
        equals(const Color(0xFFFFFFED)),
      );
    ...
    group('default themes', () {
      test('defaultLightWith', () {
        ...
        expect(newCustomization.subtitleColor, equals(const Color(0xFFFFFFFD)));
        expect(
          newCustomization.backgroundColor,
          equals(const Color(0xFFFFFFFC)),
        );
        expect(
          newCustomization.foregroundColor,
          equals(const Color(0xFFFFFFFB)),
        );
        expect(newCustomization.shadowColor, equals(const Color(0xFFFFFFFA)));
        expect(newCustomization.deleteColor, equals(const Color(0xFFFFFFF9)));
        expect(newCustomization.renameColor, equals(const Color(0xFFFFFFF8)));
    ...
    group('serialization', () {
      test('fromJson (old)', () {
        ...
        expect(
          newCustomization.pushAuthRequestAcceptColor,
          equals(const Color(0xFFFFFFF0)),
        );
        expect(
          newCustomization.actionButtonsForegroundColor,
        ...
        expect(
          newCustomization.qrButtonBackgroundColor,
          equals(const Color(0xFFFFFFEB)),
        );
        expect(
          newCustomization.qrButtonIconColor,
          equals(const Color(0xFFFFFFEA)),
        );
      ...
      test('fromJson (new)', () {
        ...
        expect(
          newCustomization.qrButtonIconColor,
          equals(const Color(0xFFFFFFE9)),
        );
        expect(
          newCustomization.tileWarningOtpColor,
          equals(const Color(0xFFFFFFE8)),
        );
        expect(
          newCustomization.tileCriticalOtpColor,
          equals(const Color(0xFFFFFFE7)),
        );
        expect(
          newCustomization.tileWarningCountdownColor,
          equals(const Color(0xFFFFFFE6)),
        );
        expect(
          newCustomization.tileCriticalCountdownColor,
          equals(const Color(0xFFFFFFE5)),
        );
      });
      test('toJson (new)', () {
        // Act
        ...
        expect(
          json['primaryColor'],
          equals({'a': 1.0, 'r': 0.0, 'g': 0.0, 'b': 0.0}),
        );
        expect(
          json['onPrimary'],
          equals({'a': 1.0, 'r': 0.0, 'g': 0.0, 'b': 0.00392156862745098}),
        );
        expect(
          json['subtitleColor'],
          equals({'a': 1.0, 'r': 0.0, 'g': 0.0, 'b': 0.00784313725490196}),
        );
        expect(
          json['backgroundColor'],
          equals({'a': 1.0, 'r': 0.0, 'g': 0.0, 'b': 0.011764705882352941}),
        );
        expect(
          json['foregroundColor'],
          equals({'a': 1.0, 'r': 0.0, 'g': 0.0, 'b': 0.01568627450980392}),
        );
        expect(
          json['shadowColor'],
          equals({'a': 1.0, 'r': 0.0, 'g': 0.0, 'b': 0.0196078431372549}),
        );
        expect(
          json['deleteColor'],
          equals({'a': 1.0, 'r': 0.0, 'g': 0.0, 'b': 0.023529411764705882}),
        );
        expect(
          json['renameColor'],
          equals({'a': 1.0, 'r': 0.0, 'g': 0.0, 'b': 0.027450980392156862}),
        );
        expect(
          json['lockColor'],
          equals({'a': 1.0, 'r': 0.0, 'g': 0.0, 'b': 0.03137254901960784}),
        );
        expect(
          json['exportColor'],
          equals({'a': 1.0, 'r': 0.0, 'g': 0.0, 'b': 0.03529411764705882}),
        );
        expect(
          json['disabledColor'],
          equals({'a': 1.0, 'r': 0.0, 'g': 0.0, 'b': 0.0392156862745098}),
        );
        expect(
          json['tileIconColor'],
          equals({'a': 1.0, 'r': 0.0, 'g': 0.0, 'b': 0.043137254901960784}),
        );
        expect(
          json['navigationBarColor'],
          equals({'a': 1.0, 'r': 0.0, 'g': 0.0, 'b': 0.047058823529411764}),
        );
        expect(
          json['warningColor'],
          equals({'a': 1.0, 'r': 0.0, 'g': 0.0, 'b': 0.07450980392156863}),
        );
        expect(
          json['successColor'],
          equals({'a': 1.0, 'r': 0.0, 'g': 0.0, 'b': 0.0784313725490196}),
        );
        expect(
          json['_pushAuthRequestAcceptColor'],
          equals({'a': 1.0, 'r': 0.0, 'g': 0.0, 'b': 0.054901960784313725}),
        );
        expect(
          json['_actionButtonsForegroundColor'],
          equals({'a': 1.0, 'r': 0.0, 'g': 0.0, 'b': 0.050980392156862744}),
        );
        expect(
          json['_tileDefaultOtpColor'],
          equals({'a': 1.0, 'r': 0.0, 'g': 0.0, 'b': 0.054901960784313725}),
        );
        expect(
          json['_tileDefaultCountdownColor'],
          equals({'a': 1.0, 'r': 0.0, 'g': 0.0, 'b': 0.24313725490196078}),
        );
        expect(
          json['_tileSubtitleColor'],
          equals({'a': 1.0, 'r': 0.0, 'g': 0.0, 'b': 0.058823529411764705}),
        );
        expect(
          json['_navigationBarIconColor'],
          equals({'a': 1.0, 'r': 0.0, 'g': 0.0, 'b': 0.06274509803921569}),
        );
        expect(
          json['_qrButtonBackgroundColor'],
          equals({'a': 1.0, 'r': 0.0, 'g': 0.0, 'b': 0.06666666666666667}),
        );
        expect(
          json['_qrButtonIconColor'],
          equals({'a': 1.0, 'r': 0.0, 'g': 0.0, 'b': 0.07058823529411765}),
        );
        expect(
          json['tileWarningOtpColor'],
          equals({'a': 1.0, 'r': 0.0, 'g': 0.0, 'b': 0.11764705882352941}),
        );
        expect(
          json['tileCriticalOtpColor'],
          equals({'a': 1.0, 'r': 0.0, 'g': 0.0, 'b': 0.1803921568627451}),
        );
        expect(
          json['tileWarningCountdownColor'],
          equals({'a': 1.0, 'r': 0.0, 'g': 0.0, 'b': 0.3058823529411765}),
        );
        expect(
          json['tileCriticalCountdownColor'],
          equals({'a': 1.0, 'r': 0.0, 'g': 0.0, 'b': 0.3686274509803922}),
        );
      });
    });
  });
