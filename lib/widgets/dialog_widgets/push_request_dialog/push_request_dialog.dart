...
class PushRequestDialog extends ConsumerStatefulWidget {
  static WidgetStateProperty<OutlinedBorder?> getButtonShape(
...
class _PushRequestDialogState extends ConsumerState<PushRequestDialog> {
  ...
  Widget build(BuildContext context) {
    ...
    final token = ref
        .watch(tokenProvider)
        .value
        ?.getTokenBySerial(widget.pushRequest.serial);
    final localizations = AppLocalizations.of(context)!;
    final title = widget.pushRequest.title == 'privacyIDEA'
        ? localizations.authentication
        : widget.pushRequest.title;
                  ...
                  PaddedRow(
                    ...
                    child: widget.pushRequest.possibleAnswers == null
                        ...
                        : RequirePresenceButtonRow(
                            ...
                            possibleAnswers:
                                widget.pushRequest.possibleAnswers!,
                            onAccept: (answer) =>
                                _onAccept(token, answer: answer),
                          ),
                  ),
  ...
  Future<void> _onAccept(PushToken token, {String? answer}) async {
    if (token.isLocked &&
        !await lockAuth(
          reason: (localization) => localization.authToAcceptPushRequest,
    ...
    final success = await ref
        ...
        .accept(token, widget.pushRequest, selectedAnswer: answer);
    if (!mounted) return;
  ...
  Future<void> _onDiscard(PushToken token) async {
    if (token.isLocked &&
        !await lockAuth(
          reason: (localization) => localization.authToDiscardPushRequest,
          localization: AppLocalizations.of(context)!,
        )) {
      return;
    }
  ...
  Future<void> _onDecline(PushToken token) async {
    if (token.isLocked &&
        !await lockAuth(
          reason: (localization) => localization.authToDeclinePushRequest,
          localization: AppLocalizations.of(context)!,
        )) {
      return;
    }
    await ref
        .read(pushRequestProvider.notifier)
        .decline(token, widget.pushRequest);
    if (mounted) setState(() => isHandled = true);
  }
}
