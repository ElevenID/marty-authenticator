...
// Note: This works because Dart doesn't restrict imports from test/ in non-test code
...
void main() async {
  Logger.init(
    ...
    appRunner: () async {
      ...
      // Log that we're running with mocks
      Logger.warning('🎭 RUNNING WITH MOCK SPRUCEID SERVICES 🎭');
      Logger.warning('This is a testing/development configuration');
      ...
      // Create mock SpruceID services with realistic configuration
      final mockServices = MockSpruceIdServices.createDefault(
        config: MockSpruceIdConfig.realistic(),
      );

      // Preload diverse credential fixtures
      mockServices.platformService.preloadFixtures();
      Logger.info('✅ Preloaded mock credential fixtures');

      // Setup mock QR scanner with all fixtures available for selection
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        return MaterialApp(
          ...
          navigatorKey: globalNavigatorKey,
          // Add mock QR scanner navigator observer
