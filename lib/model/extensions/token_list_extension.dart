...
extension TokenListExtension on List<Token> {
  ...
  List<Token> get noOffline =>
      where((token) => token.isOffline == false).toList();

  /// Returns all tokens that are privacyIDEA tokens.
  List<Token> get piTokens {
    final piTokens = where(
      (token) => token.isPrivacyIdeaToken == true,
    ).toList();
    Logger.debug(
      '${piTokens.length}/$length tokens with "isPrivacyIdeaToken == true"',
    );
    return piTokens;
  }
  ...
  List<Token> get exportableTokens {
    ...
    Logger.debug(
      '${exportableTokens.length}/$length tokens with "isPrivacyIdeaToken == false"',
    );
  ...
  List<Token> get filterNonPiTokens {
    final nonPiTokens = where(
      (token) => token.isPrivacyIdeaToken != false,
    ).toList();
    Logger.debug(
      '${nonPiTokens.length}/$length tokens with "isPrivacyIdeaToken != false"',
    );
    return nonPiTokens;
  }
  ...
  List<Token> get notLinkedTokenss {
    final maybePiTokens = where(
      (token) => token.containerSerial == null,
    ).toList();
    Logger.debug(
      '${maybePiTokens.length}/$length unlinked tokens (not in container)',
    );
  ...
  List<Token> get maybeContainerTokens {
    ...
    Logger.debug(
      '${maybeContainerToken.length}/$length tokens that could be an container',
    );
  ...
  List<Token> maybeContainerTokensOf(String containerSerial) {
    final maybeContainerTokenOf = maybeContainerTokens
        .where((token) => !token.checkedContainer.contains(containerSerial))
        .toList();
    Logger.debug(
      '${maybeContainerTokens.length}/$length tokens that could be in container $containerSerial',
    );
  ...
  List<Token> get withoutSerial =>
      where((token) => token.serial == null).toList();

  /// Returns all tokens with a specific folderId.
  List<Token> inFolder([TokenFolder? folder]) {
    if (folder == null)
      return where((token) => token.folderId != null).toList();
  ...
  List<Token> ofContainer(String containerSerial) {
    final filtered = where(
      (token) =>
          token.origin?.source == TokenOriginSourceType.container &&
          token.containerSerial == containerSerial,
    ).toList();
    Logger.debug(
      '${filtered.length}/$length tokens with containerSerial: $containerSerial',
    );
    return filtered;
  }

  /// Sorts out all tokens that type is in the given list.
  List<Token> whereNotType(List<Type> types) =>
      where((token) => !types.contains(token.runtimeType)).toList();

  List<Token> filterDuplicates() {
    final uniqueTokens = <Token>[];
    for (var token in this) {
      if (!uniqueTokens.any(
        (uniqureToken) => uniqureToken.isSameTokenAs(token) == true,
      )) {
