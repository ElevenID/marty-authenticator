...
Future<void> testCredentialReception() async {
  ...
  try {
    print('Testing credential reception from: https://trueidentityinc.azurewebsites.net/verify-credential');
    ...
    if (response.statusCode == 200) {
      ...
      print('❌ Credential verification failed with status: ${response.statusCode}');
    }
...
Future<void> testCredentialOfferReception() async {
  ...
  try {
    ...
    final offerUri = Uri.parse('openid-credential-offer://')
        .replace(queryParameters: {
      'credential_offer': jsonEncode(credentialOffer)
    });
