...
extension SpruceIdChannelHandler {
     ...
     */
    func handleOID4VPRequestRefactored(request: String, selectedCredentials: [[String: Any]], sessionId: String? = nil) async -> [String: Any] {
        print("\(Self.TAG): Handling OID4VP request with SDK integration: \(request)")

        if let sessionId = sessionId {
            return await completeOID4VPRequest(sessionId: sessionId, selectedCredentials: selectedCredentials)
        } else {
            return await initiateOID4VPRequest(request: request)
        }
    }

    private func initiateOID4VPRequest(request: String) async -> [String: Any] {
        do {
            // In a real implementation, we would parse the request using the SDK
            // let oid4vp = Oid4vp180137()
            // let parsedRequest = try oid4vp.parseRequest(request)

            // Generate session ID
            let sessionId = UUID().uuidString
            Self.pendingRequests[sessionId] = request // Store request for later

            // Return matches for UI selection
            // Simplified: returning all credentials as matches
            let matches: [[String: Any]] = [
                [
                    "id": "cred_mock_1",
                    "type": "VerifiableCredential",
                    "issuer": "did:example:issuer",
                    "requestedFields": ["name": ["given_name", "family_name"]]
                ]
            ]
            ...
            print("\(Self.TAG): OID4VP initiation failed: \(error)")
            return ["success": false, "error": error.localizedDescription]
        }
    }

    private func completeOID4VPRequest(sessionId: String, selectedCredentials: [[String: Any]]) async -> [String: Any] {
        guard let request = Self.pendingRequests.removeValue(forKey: sessionId) as? String else {
