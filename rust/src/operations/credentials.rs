...
pub(crate) fn parse_verifiable_credential(json: String) -> anyhow::Result<VerifiableCredential> {
...
pub(crate) fn parse_mdoc_credential(cbor_bytes: Vec<u8>) -> anyhow::Result<MDocCredential> {
...
pub(crate) fn parse_sd_jwt_credential(sd_jwt: String) -> anyhow::Result<SdJwtCredential> {
...
pub(crate) fn verify_mdoc_trust_chain(x5chain: Vec<Vec<u8>>) -> anyhow::Result<TrustInfo> {
    trust::verify_mdoc_trust(&x5chain)
}

pub(crate) fn verify_and_attach_trust(
...
pub(crate) fn group_credentials_by_issuer(credentials: Vec<Credential>) -> Vec<CredentialGroup> {
...
pub(crate) fn create_selectable_credential(
...
pub(crate) fn is_credential_expired(credential: &Credential) -> bool {
    credential.is_expired()
}

pub(crate) fn get_credential_claims(credential: &Credential) -> Vec<String> {
    match credential {
...
pub(crate) fn credential_to_json(credential: &Credential) -> anyhow::Result<String> {
    serde_json::to_string(credential).map_err(|e| anyhow::anyhow!("Internal error: {}", e))
}

pub(crate) fn credential_from_json(json: String) -> anyhow::Result<Credential> {
